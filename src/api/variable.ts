...
import { GetVariableList, ModifyVariable, DelVariable, GetAll, GetListById, execScript } from '../service/variable';
...
router.post('/modifyVariable', async function (ctx) {
    const { name, type, key, value, cookieDomain, scripts, isScript, id } = ctx.request.body;
    try {
        const data = await ModifyVariable('admin', name, type, key, value, isScript, scripts, cookieDomain, id);
        ctx.body = BeSuccess(data);
    } catch (error) {
        Log.debug(error.message);
        ctx.body = BeError(error.message);
...
router.post('/test', async function (ctx) {
    const { scripts } = ctx.request.body;
    try {
        const data = await execScript(scripts);
        ctx.body = BeSuccess(data);
    } catch (error) {
        Log.debug(error.message);
        ctx.body = BeError(error.message);
